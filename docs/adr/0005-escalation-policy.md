...
durable steps. The polling worker parks the structured reason, summary, exact
team, turn identity, and public Ticket context through the triggering requester
comment. The ticket adapter, rather than the model, owns the acknowledgment and
human-routing effects. PRD §6.4 specifies the current atomic handoff, retry, and
terminal-state contract. Confidence is self-reported per drafted reply with a
tunable threshold — this is the
