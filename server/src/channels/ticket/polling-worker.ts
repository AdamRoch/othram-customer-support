...
function asWorkItem(row: Record<string, unknown>): TicketWorkerItem {
  if (typeof row.status !== 'string' || !['PENDING', 'LEASED', 'READY_TO_SEND', 'REPLIED', 'ESCALATION_PENDING'].includes(row.status)) {
...
export class TicketPollingWorker {
  ...
  private async processClaimedItem(item: TicketWorkerItem): Promise<TicketWorkStatus> {
    ...
    if (!reply) {
      ...
      if (escalation) {
        await this.markEscalationPending(item, escalation);
  ...
  private async markEscalationPending(
    ...
    const changed = await this.options.database
      ...
      .set({
        ...
        escalation: { reason: event.reason, summary: event.summary, team: event.team },
        leaseToken: null,
        leaseExpiresAt: null,
        updatedAt: this.now()
      })
